...
describe(`GET ${BASE_URL}`, () => {
  it("should create event when role is ORGANIZER or ADMIN", async () => {
    const { reqAgent } = await authUser("organizer");
    const { result, payload } = await postEvent(reqAgent);
  ...
  it("should return 201 when role is USER", async () => {
